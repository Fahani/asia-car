package update_fuel_controller_test
...
func setup(vehicleWriteRepository vehicle_write_repostiroy.VehicleWriteRepository, vehicleReadRepository vehicle_read_repostiroy.VehicleReadRepository, port, path string) *http.Server {
	...
	handler := update_fuel_command.NewUpdateFuelCommandHandler(vehicleWriteRepository, vehicleReadRepository)
	controller := update_fuel_controller.NewUpdateFuelController(handler)

	http.HandleFunc(path, controller.UpdateFuel)
	go func() {
...
func TestUpdateFuel(t *testing.T) {
	...
	body := `{"device_serial_number": "abc", "fuel": 75, "update_type": "increment"}`
	...
	assert.Equal(t, 75, v.GetFuel().GetValue())


	body = `{"device_serial_number": "abc", "fuel": 5, "update_type": "decrement"}`
	resp, err = http.Post(fmt.Sprintf("http://localhost:%s%s", port, path), "application/json", bytes.NewBuffer([]byte(body)))
	assert.Nil(t, err)
