package update_battery_controller_test
...
func setup(vehicleWriteRepository vehicle_write_repostiroy.VehicleWriteRepository, vehicleReadRepository vehicle_read_repostiroy.VehicleReadRepository, port, path string) *http.Server {
	...
	handler := update_battery_command.NewUpdateBatteryLevelCommandHandler(vehicleWriteRepository, vehicleReadRepository)
	controller := update_battery_controller.NewUpdateBatteryController(handler)

	http.HandleFunc(path, controller.UpdateBattery)
	go func() {
...
func TestUpdateBatteryLevel(t *testing.T) {
	port := "9005"
	path := "/vehicles/update-battery"
	...
	body := `{"device_serial_number": "abc", "battery": 75}`
	...
	assert.Equal(t, 75, v.GetBatteryLevel().GetValue())
}
